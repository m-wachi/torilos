...
void mymain() {
	...
	int fd_intcntr = 0, fd_timer, ctrl_status=0;
	int redisp_pos = 0;
	...
	//PAG_PGM_PAGE_INFO app05_pginfo, app06_pginfo;
	//PAG_VM_MAP_ITEM app05_pginfo, app06_pginfo;
	PAG_VM_INF app05_vminf, app06_vminf;
	PAG_VM_INF* p_app05_1_vminf;
	...
	TFLM_HND_FSMDL hnd_fsmdl;	// FileSystem Handle (wrapper version)
	...
	/* setup main task */
	//task0 = tsk_new_task(2*8, 1*8, 0, 0x280000, 0, 0);
	task0 = tsk_new_task(2*8, 1*8, 0, 0x280000);

	puts_varval01(&hnd_sc, &my_cnsl, "task0 selector", task0->sel, 1);
	...
	for (;;) {
		...
		if (cur_sec != prev_sec) {
			...
			boxfill8(hnd_sc.vram_base, hnd_sc.width, COL8_DARK_SKYBLUE, 
		...
		if (0 < fifo8_count(&g_keyque)) {
			...
			if (data == 0x48) { // up arrow
				...
				if (kbdchar == 0x0a) {
					my_strcpy(cmdbuf, hnd_lipt.line);
				} else {
					cmdbuf[0] = '\0';
				}
		...
		*/
		/*
			...
			puts_varval01(&hnd_sc, &my_cnsl, "tss_b addr", (int)&tss_b, 1);
			puts_varval01(&hnd_sc, &my_cnsl, "tss_b.eip", tss_b.eip, 1);
			puts_varval01(&hnd_sc, &my_cnsl, "tss_b.esp", tss_b.esp, 1);
			puts_varval01(&hnd_sc, &my_cnsl, "tss_b.ss", tss_b.ss, 1);

			debug_puts("switch task c");
			io_sti();
			start_apptask(0, 6*8, esp0addr);

			debug_puts("switch back from task c");
			ctrl_status=100;
		...
		*/
			...
			//rootdir_ent = prepare_filesystem(&hnd_fs);
			//rootdir_ent = prepare_filesystem(&hnd_fsmdl);
			...
			puts_varval01(&hnd_sc, &my_cnsl, "file size", 
					user_fdtbl[fd]->inode->i_size, 1);
			
			my_memset(buff, 0, sizeof(buff));
			
			sz_read = tflm_read(fd, buff, 500, user_fdtbl);
			
			puts_varval01(&hnd_sc, &my_cnsl, 
					"sz_read", sz_read, 0);
			
			my_puts(&hnd_sc, &my_cnsl, "print hello2.txt.");
		...
		*/
			...
			if (fd > 0) {
				...
				tfl_write(fd, buff, 1024, user_fdtbl);
...
//		}
			...
			put_multi(&hnd_sc, &my_cnsl, databuf);
		...
		*/
		...
		else if (g_time_counter > 5200 && ctrl_status==22) {
			unsigned int pde_addr = 0x60000;
			int rc;

			/*
		  puts_varval01(&hnd_sc, &my_cnsl, "pde_addr", 
						  pde_addr, 1);

			rc = test_pag(pde_addr, 0x480000, 0x482000, 4);

			if(rc < 0) {
				puts_varval01(&hnd_sc, &my_cnsl, "error: rc", 
							  rc, 0);
			}
			*/
			...
			rc = tatam_readblock((char*)0x500000, 0x48);
			puts_varval01(&hnd_sc, &my_cnsl, 
